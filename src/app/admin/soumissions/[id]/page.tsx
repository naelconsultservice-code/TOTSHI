            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl p-5">
              ...
              <div className="flex flex-col gap-2">
                {submission.statusHistory.length === 0 ? (
                  <p className="text-gray-600 text-xs">Aucun historique</p>
                ) : (
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl p-5">
              <h2 className="text-white font-semibold mb-4 text-sm">
                {needType === 'MARKETING' ? 'Cible marketing' : 'Utilisateurs cibles'}
              </h2>
            ...
            {features?.selected?.length > 0 && (
              <div className="bg-white/3 border border-white/8 rounded-2xl p-5">
                <h2 className="text-white font-semibold mb-4 text-sm">
                  {needType === 'MARKETING' ? 'Prestations souhaitées'
                    : needType === 'CONSULTANCE' ? 'Livrables attendus'
                    : 'Fonctionnalités'}
                </h2>
            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  { label: 'Références', value: constraints?.references },
                ].map(({ label, value }) => (
                  <div key={label}>
                    <p className="text-gray-600 text-xs mb-1">{label}</p>
                    <p className="text-gray-300">{value || '—'}</p>
              ...
              <div className="bg-[#0066FF]/5 border border-[#0066FF]/20 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-white font-semibold text-sm">
                    {needType === 'MARKETING' ? 'Plan marketing IA'
                      : needType === 'CONSULTANCE' ? 'Proposition de mission IA'
                ...
                {draft.generationStatus === 'GENERATING' && (
                  <p className="text-yellow-400 text-sm">⏳ Génération IA en cours...</p>
                )}
                {draft.generationStatus === 'PENDING' && (
                  <p className="text-gray-400 text-sm">⏳ En attente de génération...</p>
                )}
              </div>
            )}
          </div>
